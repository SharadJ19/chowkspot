...
def main():
    ...
    print(
        f"\n{COLOR_HEADER}{COLOR_BOLD}=== ChowkSpot Frontend Context Copy Engine ==={COLOR_RESET}\n"
    )
    ...
    if copy_to_clipboard_native(full_text):
        print(
            f"{COLOR_GREEN}{COLOR_BOLD}🚀 SUCCESS! {len(processed_files)} frontend files copied directly to clipboard.{COLOR_RESET}\n"
        )
    else:
        print(
